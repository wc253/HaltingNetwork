...
# tao=100
# eps=[0.7,0.1,0.09,0.08,0.07,0.06,0.05,0.04,0.03,0.028,0.024,0.02,0.018,0.014,0.01,0.009,0.008,0.007,0.006,0.0055,0.005,0.0045,0.004,0.0035,0.003,0.002,0.001]
# tao=1
# eps=[1,0.7,0.6,0.5,0.4,0.3,0.2,0.18,0.16,0.14,0.12,0.1,0.09,0.08,0.07,0.06,0.05,0.04,0.03,0.02,0.01,0.006]
# eps=[0.0001]

tao=10
eps=[0.3,0.16,0.12,0.1,0.09,0.08,0.07,0.06,0.05,0.048,0.046,0.044,0.042,0.04,0.038,0.036,0.034,0.032,0.03,0.028,0.026,0.024,0.022,0.02,0.018,0.016,0.014,0.0133]#,0.095,0.09,0.085,0.07,0.065,0.06,0.055,0.05,0.01
...
for i in range(len(eps)):
    ...
    layers = networks.build_LISTA_act(prob,tao,T=T,eps=eps[i],initial_lambda=.1,untied=False)
    savefile = 'LISTA_T_'+str(T)+'_tao_' + str(tao) + '_count_'+str(count)+'.npz'
...
sio.savemat('LISTA_'+str(T)+'_layer_noise_free_tao_'+str(tao)+'_'+str(count)+'_mse_test',{'layer':xs,'loss':ys_mean,'loss_var':ys_var,'loss_std':ys_std,'loss_total':loss_total})
