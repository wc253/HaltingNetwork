...
def shrink_ss (inputs_, theta_, q):
    ...
    # index2_=tf.to_float(thresh>tf.ones_like(thresh)*1e-2)
    # cindex2_=1.0-index2_
    return tf.multiply (index_, inputs_) +shrink_free (tf.multiply (cindex_, inputs_), theta_ )
...
def shrink_bgest(r,rvar,theta):
    ...
    beta = 1/(1+rvar/(xvar1+1e-10))
    r2scale = r*r*beta/(rvar+1e-10)
    rho = tf.exp(loglam - .5*r2scale ) * tf.sqrt(1 +xvar1/(rvar+1e-10))
    rho1 = rho+1
    xhat = beta*r/rho1
