...
for i in range(17):
  ...
  if i>=1:
    layers = networks.build_LISTA(prob,T=i,initial_lambda=.1,untied=False)
  else:
    layers = networks.build_LISTA(prob,T=1,initial_lambda=.1,untied=False)  
...
# do the learning (takes a while)
  if i>=2:
    sess = train.do_training(training_stages,prob,'LISTA T='+str(i-1)+' trainrate=0.001.npz',printfile='layer_'+str(i)+'.txt') #'LISTA T='+str(i-1)+' trainrate=0.001.npz'
  else:
    sess = train.do_training(training_stages,prob,'Linear trainrate=0.001.npz',printfile='layer_'+str(i)+'.txt')
